// GET/POST /api/tecnicos - Technicians management (tenant + location scoped)
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const r = await env.DB.prepare(
      `SELECT * FROM tecnicos WHERE tenant_id = ? AND location_id = ? ORDER BY nombre ASC`
    ).bind(tenantId, locationId).all();

    return json({ success: true, tecnicos: r.results });
  } catch (err) {
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const res = await env.DB.prepare(`
      INSERT INTO tecnicos (tenant_id, location_id, nombre, telefono, email, especialidad, estado, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `).bind(
      tenantId,
      locationId,
